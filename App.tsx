...
export default function App() {
  ...
  const [editingExpense, setEditingExpense] = useState<JoinedExpense | null>(
    null,
  );
  ...
  const handlePasswordUpdate = useCallback(
    async (currentPassword: string, newPassword: string) => {
      if (!user) {
        throw new Error("錯誤：無法識別使用者身份，請重新登入。");
      }
      ...
      try {
        ...
        throw new Error(err.message || "密碼更新失敗");
      }
    },
    [user],
  );
  ...
  const handleAccountDelete = useCallback(async () => {
    ...
    if (
      !window.confirm("您確定要永久刪除您的帳號及所有資料嗎？此操作無法復原。")
    ) {
      return;
    }
  ...
  const handleAddExpense = useCallback(
    async (expense: Omit<Expense, "id" | "created_at" | "user_id">) => {
      if (!user) return;
      const { error } = await supabase
        .from("expenses")
  ...
  const handleUpdateExpense = useCallback(
    async (
      expenseId: string,
      updatedData: Omit<Expense, "id" | "created_at" | "user_id">,
    ) => {
      if (!user) return;
      const { error } = await supabase
        .from("expenses")
  ...
  return (
    <div className="min-h-screen">
      <Header
        appName={appName}
        onAppNameChange={handleAppNameUpdate}
            ...
            <div className={activeTab === "records" ? "block" : "hidden"}>
              <div className="grid grid-cols-1 lg:grid-cols-3 xl:grid-cols-4 gap-8">
                <div className="lg:col-span-1 xl:col-span-1 space-y-8">
                  ...
                  />
                  <CalendarView
                    expenses={joinedExpenses}
                    onDateSelect={openModalForDate}
